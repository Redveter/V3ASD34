...
on:
  workflow_dispatch:
    inputs:
      INSTANCE:
        description: "⌨️ Numero de instancia [1-10]:"
        required: true
        default: "1"
        type: string
...
jobs:
  deploy-enigmano:
    ...
    steps:
      ...
      - name: 🪙 Extensión - uBlock Origin (Brave)
        shell: powershell
        run: |
        ...
        run: |
          ...
          if (-not $scriptPath) {
            if (-not $env:SCRIPT_URL) {
              Write-Error "No se encontró 'EnigMano-instance.ps1' local y SCRIPT_URL no está definido."
            ...
            try {
              ...
              Write-Host "Descarga directa falló. Intentando vía API de GitHub..."
