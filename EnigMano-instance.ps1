...
try {
    Log ("Aplicando wallpaper directo en el perfil de {0} (sin RunOnce ni tareas)" -f $Username)

    # Carpeta de temas DENTRO del perfil del usuario destino
    $userProfile = Join-Path $env:SystemDrive ("Users\{0}" -f $Username)
    $wpRoot     = Join-Path $userProfile 'AppData\Roaming\Microsoft\Windows\Themes'
    New-Item -Path $wpRoot -ItemType Directory -Force | Out-Null

    # Descargar imagen al perfil del usuario
    ...
    Log ("Destino de wallpaper: {0}" -f $wpPath)
    $headers = @{ 'User-Agent' = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, como Gecko) Chrome/127.0 Safari/537.36' }
    Invoke-WebRequest -Uri $WALLPAPER_URL -Headers $headers -OutFile $wpPath -UseBasicParsing -ErrorAction Stop
    Log "Wallpaper descargado"

    if (Test-Path $wpPath) {
        # Cargar hive del usuario objetivo y escribir configuracion (HKU\<SID>)
        $userNt = Join-Path $userProfile 'NTUSER.DAT'
        if (Test-Path $userNt) {
            try {
                $sid = $targetSid
                if (-not $sid) { $sid = (New-Object System.Security.Principal.NTAccount($Username)).Translate([System.Security.Principal.SecurityIdentifier]).Value }
                ...
                Log ("Perfil de {0} preconfigurado con wallpaper (HKU)" -f $Username)
            } catch {
            ...
            Log ("NTUSER.DAT no existe aun en {0}; se aplicara al crearse el perfil (CreateProfile ya intentado)" -f $userProfile)
        }

        # Generar TranscodedWallpaper dentro del perfil del usuario
        try {
            $transcoded = Join-Path $wpRoot 'TranscodedWallpaper'
            Copy-Item -Path $wpPath -Destination $transcoded -Force
            Log "TranscodedWallpaper creado"
        } catch {
            Log ("No se pudo crear TranscodedWallpaper: {0}" -f $_.Exception.Message)
        }

        # Aplicación inmediata solo si YA estamos en sesion de $Username (no programar nada)
        try {
            if ($env:USERNAME -and ($env:USERNAME -ieq $Username)) {
                ...
                Set-ItemProperty -Path $reg -Name Wallpaper -Value $wpPath
                ...
                [Win32.Native]::SystemParametersInfo(0x0014, 0, $wpPath, 0x0001 -bor 0x0002) | Out-Null
                rundll32 user32.dll,UpdatePerUserSystemParameters 1,True

                Log ("Wallpaper aplicado inmediatamente en HKCU para {0}" -f $Username)
            } else {
                Log ("Wallpaper listo en el perfil de {0}; se mostrará al iniciar sesión" -f $Username)
            }
        } catch {
