...
# === ACCESS ENABLEMENT ===
 ...
 try {
     ...
     # Log de diagnostico para evitar 'Path null'
     Log ("wpRoot: {0}" -f $wpRoot)
     Log ("wpPath: {0}" -f $wpPath)
     ...
     if (Test-Path $wpPath) {
         ...
         if ([string]::IsNullOrWhiteSpace($applyScriptPath)) { throw "applyScriptPath invalido" }
         $psApplyContent = @"
 param()
 try {
   $img    = '$wpPath'
 ...
 "@
         ...
         Log ("Script de refuerzo creado: {0}" -f $applyScriptPath)
